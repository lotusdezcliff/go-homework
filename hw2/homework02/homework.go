...
func TaskScheduler(tasks []func()) {
	// TODO: 实现代码
	var wg sync.WaitGroup
	wg.Add(len(tasks)) // number of tasks
	for i, task := range tasks {
		go func(taskID int, taskFunc func()) {
			defer wg.Done()
			start := time.Now() // record start time
			taskFunc()          // run
			duration := time.Since(start) // compute time
...
func (e Employee) PrintInfo() string {
	// TODO: 实现代码
	return fmt.Sprintf("ID: %s | Name: %s | Age: %d", e.EmployeeID, e.Name, e.Age)
}
...
func ProducerConsumer() []int {
	...
	for num := range ch {
		result = append(result, num) // receive num and record
	}
	return result
}
